...
SELECT pt.project_id, pt.is_geocoded, pt.project_title, pt.start_actual_isodate, pt.end_actual_isodate,pt.donors, pt.donors_iso3,cc.id,pt.ad_sector_codes,pt.ad_sector_names,s.id,pt.transactions_start_year,pt.transactions_end_year,CAST(pt.total_commitments AS DECIMAL(19,5)),CAST(pt.total_disbursements AS DECIMAL(19,5))
FROM projects_temp as pt, status as s, countrycodes as cc
WHERE pt.recipients = cc.name AND pt.status = s.name;
